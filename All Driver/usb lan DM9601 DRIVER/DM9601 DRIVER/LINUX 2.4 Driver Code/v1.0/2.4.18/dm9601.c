...
static int get_registers(dm9601_board_info_t *dbi, __u16 indx, __u16 size, void *data)
{
	...
	dbi->dr.requesttype = DM9601_REQT_READ;
	dbi->dr.request = DM9601_REQ_GET_REGS;
	dbi->dr.value = cpu_to_le16 (0);
	dbi->dr.index = cpu_to_le16p(&indx);
	dbi->dr.length = cpu_to_le16p(&size);
...
static int set_registers(dm9601_board_info_t *dbi, __u16 indx, __u16 size, void *data)
{
	...
	dbi->dr.requesttype = DM9601_REQT_WRITE;
	dbi->dr.request = DM9601_REQ_SET_REGS;
	dbi->dr.value = cpu_to_le16(0);
	dbi->dr.index = cpu_to_le16p(&indx);
	dbi->dr.length = cpu_to_le16p(&size);
	dbi->ctrl_urb.transfer_buffer_length = size;
...
static int set_register( dm9601_board_info_t *dbi, __u16 indx, __u8 data )
{
	...
	dbi->dr.requesttype = DM9601_REQT_WRITE;
	dbi->dr.request = DM9601_REQ_SET_REG;
	dbi->dr.value = cpu_to_le16p( &dat);
	dbi->dr.index = cpu_to_le16p( &indx );
	dbi->dr.length = cpu_to_le16( 0 );
...
static int update_eth_regs_async( dm9601_board_info_t *dbi )
{
	...
	if (dbi->flags & HASH_REGS_CHANGE) {
		...
		dbi->dr.request = DM9601_REQ_SET_REGS;
		dbi->dr.value = cpu_to_le16(0);
		dbi->dr.index =  cpu_to_le16(0x16);
		dbi->dr.length = cpu_to_le16(8);
		...
		dbi->dr.requesttype = DM9601_REQT_WRITE;
		dbi->dr.request = DM9601_REQ_SET_REG;
		dbi->dr.value = cpu_to_le16(dbi->rx_ctrl_reg);
		dbi->dr.index =  cpu_to_le16(0x5);
		dbi->dr.length = cpu_to_le16(0);
		dbi->ctrl_urb.transfer_buffer_length = 0;
		...
		dbi->dr.requesttype = DM9601_REQT_WRITE;
		dbi->dr.request = DM9601_REQ_SET_REG;
		dbi->dr.value = cpu_to_le16(dbi->net_ctrl_reg);
		dbi->dr.index =  cpu_to_le16(0x0);
		dbi->dr.length = cpu_to_le16(0);
		dbi->ctrl_urb.transfer_buffer_length = 0;
...
static void set_phy_mode(dm9601_board_info_t *dbi)
{
	...
	if ( !(dbi->op_mode & DM9601_AUTO) ) {	
		switch(dbi->op_mode) {
		case DM9601_10MHF:  phy_reg4 = 0x0021; break;
		case DM9601_10MFD:  phy_reg4 = 0x0041; break;
		case DM9601_100MHF: phy_reg4 = 0x0081; break;
		case DM9601_100MFD: phy_reg4 = 0x0101; break;
		default: phy_reg0 = 0x8000; break;
		}
